...
model = dict(
    ...
    lora=dict(
        ...
        drop_rate=0.,
        targets=[
                    dict(type='ffn',alpha=32,rank=32),
                    dict(type='attn',alpha=32,rank=32)
                ]
    ),

    backbone=dict(
        type='Dinov2VisionTransformer_Conv_Lora',
        arch='base',
        img_size=1024,
        patch_size=16,
        layer_scale_init_value=1e-5,
        out_type = 'featmap',
        init_cfg=dict(type='Pretrained', checkpoint='/dataset/changed_vit-base-p14to16_dinov2-pre_3rdparty_20230426-ba246503.pth')
    ),
     neck=dict(
