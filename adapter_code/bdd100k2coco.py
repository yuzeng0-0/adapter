...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-s",
        "--save_path",
        default="data/bdd100k/annotations/",
...
def main() -> None:
    ...
    with open(
        os.path.join(args.label_dir, "det_train.json")
    ) as f:
        train_labels = json.load(f)
    print("Converting training set...")

    out_fn = os.path.join(
        args.save_path, "det_train.json"
    )
    ...
    with open(
        os.path.join(args.label_dir, "det_val.json")
    ) as f:
        val_labels = json.load(f)
    print("Converting validation set...")

    out_fn = os.path.join(
        args.save_path, "det_val.json"
    )
...
if __name__ == "__main__":
    import os
    os.makedirs('data/bdd100k/annotations', exist_ok=True)
    main()