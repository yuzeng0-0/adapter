...
class SwinAttentionWithRoPE(BaseModule):
    ...
    def __init__(self,
                 ...
                 with_cls_token=True,
                 init_cfg=None):
        super(SwinAttentionWithRoPE, self).__init__(init_cfg=init_cfg)
...
class SwinEVA02EndcoderLayer(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(SwinEVA02EndcoderLayer, self).__init__(init_cfg=init_cfg)

        self.norm1 = build_norm_layer(norm_cfg, embed_dims)
        self.window_size = window_size
        # with_cls_token 在swin中默认为False
        self.attn = SwinAttentionWithRoPE(
    ...
    def forward(self, x, patch_resolution):
        ...
        if self.window_size > 0:
            ...
            x = self.attn(x,self.window_size)
        else:
...
class SwinViTEVA02(Dinov2VisionTransformer):
    ...
    def __init__(self,
        ...
        kwargs.update(
            ...
            norm_cfg=norm_cfg,
            with_cls_token=with_cls_token,
             is_resize_rope = is_resize_rope
            )
        super(SwinViTEVA02, self).__init__(**kwargs)
        ...
        for i in range(self.num_layers):
            _layer_cfg = dict(
                ...
                with_cls_token=False if i not in global_attn_indexes else True,
                drop_path_rate=dpr[i])
            _layer_cfg.update(layer_cfgs[i])
            self.layers.append(SwinEVA02EndcoderLayer(**_layer_cfg))
