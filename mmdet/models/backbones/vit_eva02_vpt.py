...
class AttentionWithRoPE(BaseModule):
    ...
    def forward(self, x, patch_resolution):
        ...
        q, k, v = qkv.unbind(dim=0)    # [bs,num_head,num_patch,embed]

        if self.rope:
            if self.with_cls_token:
                q_t = q[:, :, 1+self.num_vpt_token:, :]
                ro_q_t = self.rope(q_t, patch_resolution)
                q = torch.cat((q[:, :, :1+self.num_vpt_token, :], ro_q_t), -2).type_as(v)

                k_t = k[:, :, 1+self.num_vpt_token:, :] if self.with_cls_token else k
                ro_k_t = self.rope(k_t, patch_resolution)
                k = torch.cat((k[:, :, :1+self.num_vpt_token, :], ro_k_t), -2).type_as(v)
            else:
...
class ViTEVA02VPT(Dinov2VisionTransformerVPT):
    ...
    def __init__(self,
        ...
        super(ViTEVA02VPT, self).__init__(**kwargs)
