...
from ..utils import (MultiheadAttentionPrefix, SwiGLUFFNFused, build_norm_layer,
...
class TransformerEncoderLayerPrefix(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(TransformerEncoderLayerPrefix, self).__init__(init_cfg=init_cfg)

        self.embed_dims = embed_dims

        self.ln1 = build_norm_layer(norm_cfg, self.embed_dims)

        # 之前的MultiheadAttention不加prefix，正常计算
        # 在../utils/attention.py文件中新加了一个MultiheadAttentionPrefix的类
        self.attn = MultiheadAttentionPrefix(
    ...
    def init_weights(self):
        super(TransformerEncoderLayerPrefix, self).init_weights()
    ...
    def forward(self, x, prefix_token_k, prefix_token_v):
        x = x + self.attn(self.ln1(x), prefix_token_k, prefix_token_v)
        x = self.ffn(self.ln2(x), identity=x)
        return x


@MODELS.register_module()
class Dinov2VisionTransformerPrefix(BaseModule):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(Dinov2VisionTransformerPrefix, self).__init__(init_cfg)
        ...
        # prefix 配置
        # 根据num_layers来生成对应的参数，然后传入每一层encoder layer
        # dino 和 encoder的forward多传一个prefix token参数
        # 对于encoder的attention forward做修改
        self.prefix_tokens_num = prefix_config['prefix_tokens_num']
        self.prefix_token_k = nn.Parameter(
                    torch.zeros(self.num_layers, self.prefix_tokens_num, self.embed_dims)
                )
        ...
        for i in range(self.num_layers):
            ...
            self.layers.append(TransformerEncoderLayerPrefix(**_layer_cfg))
    ...
    def init_weights(self):
        super(Dinov2VisionTransformerPrefix, self).init_weights()
    ...
    def _freeze_stages(self):
        ...
        if self.pos_embed is not None and self.pos_tune==True:
        ...
        for i, layer in enumerate(self.layers):
            ...
            for name,_ in layer.named_modules():
                if 'adapter' in name:
                    ...
                    break
            if ori_forward:
                x = layer(x, self.prefix_token_k[i], self.prefix_token_v[i])
            else:
                x = layer(x, patch_resolution, self.prefix_token_k[i], self.prefix_token_v[i])
