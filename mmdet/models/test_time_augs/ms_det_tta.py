...
class MSDetTTAModel(BaseTTAModel):
    ...
    def __init__(self, tta_cfg=None,
        ...
        self.is_convert_box = is_convert_box
    ...
    def merge_aug_bboxes(self, aug_bboxes: List[Tensor],
        ...
        for bboxes, img_info in zip(aug_bboxes, img_metas):
            ''' 
            NOTE:在detr_head.py 中的 _predict_by_feat_single函数已经做了相同的处理,因此这里什么都不用做,只用将aug box cat起来做nms即可
            '''
            # scale_factor = img_info['scale_factor']
            
            # # w_scale = img_metas['scale_factor'][0]
            # # h_scale = img_metas['scale_factor'][1]
            
            # assert len(scale_factor) == 2
            # scale_factor = bboxes.new_tensor(scale_factor).repeat(2)
            # if self.is_convert_box:
            
            #     # 预测出的box是xywh,gt是xyxy
            #     bboxes = bbox_cxcywh_to_xyxy(bboxes)
            #     bboxes = bboxes / scale_factor
            #     ori_shape = img_info['ori_shape']   
            #     bboxes[..., 0::2] = bboxes[..., 0::2].clamp(0, ori_shape[1])
            #     bboxes[..., 1::2] = bboxes[..., 1::2].clamp(0, ori_shape[0])
