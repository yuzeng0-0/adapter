...
class DINOHeadv2(DeformableDETRHead):
    ...
    #             lambda_1 = 1,
    #              **kwargs) -> None:
    #     super(CoDINOHead,self).__init__(*args, **kwargs)
    ...
    def loss_by_feat(
        ...
        loss_dict = self.detr_loss_by_feat(
            all_layers_matching_cls_scores, all_layers_matching_bbox_preds,
            batch_gt_instances, batch_img_metas, batch_gt_instances_ignore)
    ...
    def detr_loss_by_feat(
        ...
        """"Loss function.
        copy from detr_head.py
        change Focal loss to QualityFocalLoss
    ...
    def detr_loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
                            batch_gt_instances: InstanceList,
                            batch_img_metas: List[dict]) -> Tuple[Tensor]:
        """
            copy from detr_head.py  loss_by_feat_single
        """
        ...
        # NOTE 由 原来的Focal loss 改为 QualityFocalLoss
        bg_class_ind = self.num_classes
        pos_inds = ((labels >= 0)
                    & (labels < bg_class_ind)).nonzero().squeeze(1)
        scores = label_weights.new_zeros(labels.shape)
