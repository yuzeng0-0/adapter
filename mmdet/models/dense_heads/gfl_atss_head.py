...
class GFLATSSHead(AnchorHead):
    ...
    def __init__(self,
                 ...
                 gfl_bbox_coder: ConfigType = dict(type='DistancePointBBoxCoder'),
        ...
        self.gfl_bbox_coder = TASK_UTILS.build(gfl_bbox_coder)
    ...
    def forward_single(self, x: Tensor, scale: Scale) -> Sequence[Tensor]:
        ...
        gfl_bbox_pred = scale(self.gfl_reg(reg_feat)).float()
        centerness = self.atss_centerness(reg_feat)
        return cls_score, bbox_pred,centerness,gfl_bbox_pred

    def loss_by_feat_single(self, anchors: Tensor, cls_score: Tensor,
                            bbox_pred: Tensor,gfl_bbox_pred:Tensor, centerness: Tensor,
                            labels: Tensor, label_weights: Tensor,
                            bbox_targets: Tensor,stride: Tuple[int],avg_factor: float) -> dict:
        ...
        # for gfl
        gfl_bbox_pred = gfl_bbox_pred.permute(0, 2, 3,
        ...
        if len(pos_inds) > 0:
            ...
            gfl_pos_bbox_pred = gfl_bbox_pred[pos_inds]
            pos_bbox_pred_corners = self.integral(gfl_pos_bbox_pred)
            gfl_pos_decode_bbox_pred = self.gfl_bbox_coder.decode(
                pos_anchor_centers, pos_bbox_pred_corners)
            gfl_pos_decode_bbox_targets = pos_bbox_targets / stride[0]
            score[pos_inds] = bbox_overlaps(
                gfl_pos_decode_bbox_pred.detach(),
                gfl_pos_decode_bbox_targets,
                is_aligned=True)
            gfl_pred_corners = gfl_pos_bbox_pred.reshape(-1, self.reg_max + 1)
            gfl_target_corners = self.gfl_bbox_coder.encode(pos_anchor_centers,
                                                    gfl_pos_decode_bbox_targets,
        ...
        return loss_cls, loss_bbox, loss_centerness, loss_dfl,centerness_targets.sum()
    ...
    def loss_by_feat(
        ...
        losses_cls, losses_bbox, loss_centerness, losses_dfl,\
    ...
    def predict(self,
        ...
        """
        '''
        重写父类的方法,因为outs中包含了cls_score, bbox_pred,centerness,gfl_bbox_pred
        '''
