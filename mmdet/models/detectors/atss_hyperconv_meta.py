...
from mmdet.models.adapters.hypernet_convpass_metanet import Convpass_swin_hypernet_fuse,HyperNetwork_conv,Convpass_swin_hypernet_fusev2,Convpass_swin_hypernet_fusev3
from mmdet.models.adapters.convpass import forward_sam_block_t1,forward_dinov2_block_t1,forward_eva02_block_t1
import torch.nn as nn

@MODELS.register_module()
class ATSSHyperConvPass_metanet(SingleStageDetector):
            ...
            for child_module in tuning_module.children():
                if type(child_module) == mmengine.model.base_module.ModuleList:
                    for idx,layer in enumerate(child_module):
                        if self.share_linear:
                            layer.adapter_attn = Convpass_swin_hypernet_fusev2(self.adapter_down,self.adapter_up,self.adapter_attn_conv_hypetnet,self.layer_embedding.weight[idx],self.backbone_embed_dim,self.dim,self.fuse_type).to(layer.norm1.weight.device)
                            layer.adapter_mlp =  Convpass_swin_hypernet_fusev2(self.adapter_down,self.adapter_up,self.adapter_mlp_conv_hypetnet,self.layer_embedding.weight[idx],self.backbone_embed_dim,self.dim,self.fuse_type).to(layer.norm1.weight.device)
                            layer.s = self.scale
                            bound_method = forward_sam_block_t1.__get__(layer, layer.__class__)
                            setattr(layer, 'forward', bound_method)
                        elif self.hyper_linear:
                            layer.adapter_attn = Convpass_swin_hypernet_fusev3( self.adapter_attn_down_lin_hypetnet,self.adapter_attn_up_lin_hypetnet,self.adapter_attn_conv_hypetnet,self.layer_embedding.weight[idx],self.backbone_embed_dim,self.dim,self.fuse_type).to(layer.norm1.weight.device)
                            layer.adapter_mlp =  Convpass_swin_hypernet_fusev3(self.adapter_mlp_down_lin_hypetnet,self.adapter_mlp_up_lin_hypetnet,self.adapter_mlp_conv_hypetnet,self.layer_embedding.weight[idx],self.backbone_embed_dim,self.dim,self.fuse_type).to(layer.norm1.weight.device)
                            layer.s = self.scale
                            bound_method = forward_sam_block_t1.__get__(layer, layer.__class__)
                            setattr(layer, 'forward', bound_method)
                            ...
                            layer.adapter_attn = Convpass_swin_hypernet_fuse(self.adapter_attn_conv_hypetnet,self.layer_embedding.weight[idx],self.backbone_embed_dim,self.dim,self.fuse_type).to(layer.norm1.weight.device)
                            layer.adapter_mlp =  Convpass_swin_hypernet_fuse(self.adapter_mlp_conv_hypetnet,self.layer_embedding.weight[idx],self.backbone_embed_dim,self.dim,self.fuse_type).to(layer.norm1.weight.device)
                            layer.s = self.scale
                            bound_method = forward_sam_block_t1.__get__(layer, layer.__class__)
                            setattr(layer, 'forward', bound_method)
