...
from mmdet.models.adapters.hypernet_lora import HyperLoRALinear,LoraHypernet

@MODELS.register_module()
class ATSSHyperLora(SingleStageDetector):
    ...
    def __init__(self,
        ...
        if lora is not None:
            ...
            self.hyper_net_insize = lora['in_size']
            self.num_mask = lora['num_mask']
            self.lora_mask_token = nn.Embedding(self.num_mask, self.hyper_net_insize)
    ...
    def apply_lora(self,tuning_module):
        ...
        '''
        self.lora_hypernet_1 = LoraHypernet(self.hyper_net_insize,self.backbone_embed_dim,self.rank)
        self.lora_hypernet_2 = LoraHypernet(self.hyper_net_insize,self.backbone_embed_dim*4,self.rank)
    ...
    def _replace_module(self, tuning_module,full_module_name: str, _child_module: nn.Module,
        ...
        # for name,_child_module in current_module.named_modules():
        #     if _child_module.__class__.__name__ == "Linear":
        if _child_module.in_features == self.backbone_embed_dim:
            target_module = HyperLoRALinear(self.lora_hypernet_1,self.lora_mask_token,_child_module, alpha, rank, drop_rate).to(_child_module.weight.device)
        else:
            target_module = HyperLoRALinear(self.lora_hypernet_2,self.lora_mask_token,_child_module, alpha, rank, drop_rate).to(_child_module.weight.device)
