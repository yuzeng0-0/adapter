...
from mmdet.models.adapters.hom_selayer import *

@MODELS.register_module()
class ATSSHomSE(SingleStageDetector):
    ...
    def __init__(self,
        ...
        self.apply_hom_se(self.backbone)
        self._set_convpass_trainable(self.backbone)
            
            
    def apply_hom_se(self,tuning_module):
                ...
                if type(child_module) == mmengine.model.base_module.ModuleList:
                    for layer in child_module:
                        if tuning_module.__class__.__name__ == 'SAMImageEncoderViTv3':
                            layer.adapter_attn = Hom_SE(tuning_module.embed_dim,self.dim, False,'xavier').to(layer.norm1.weight.device)
                            layer.adapter_mlp = Hom_SE(tuning_module.embed_dim,self.dim, False,'xavier').to(layer.norm1.weight.device)
                            layer.s = self.scale
