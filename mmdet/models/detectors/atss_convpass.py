...
class ATSSConvPass(SingleStageDetector):
    ...
    def __init__(self,
        ...
        if convpass is not None:
            ...
            self.linear_init = convpass['linear_init']
            
            if 'low_dim' in convpass.keys():
                self.low_dim = convpass['low_dim']
            if 'order' in convpass.keys():
                self.order = convpass['order']
                ...
                if type(child_module) == mmengine.model.base_module.ModuleList:
                    for layer in child_module:
                        if tuning_module.__class__.__name__ == 'SAMImageEncoderViTv3':
                            layer.adapter_attn = Convpass_swin(tuning_module.embed_dim,self.dim, self.xavier_init,self.linear_init).to(layer.norm1.weight.device)
                            layer.adapter_mlp = Convpass_swin(tuning_module.embed_dim,self.dim, self.xavier_init,self.linear_init).to(layer.norm1.weight.device)
                            layer.s = self.scale
