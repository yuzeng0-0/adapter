...
from mmdet.models.adapters.hypernet_convpass import HyperNetwork_conv,Convpass_swin_hypernet,Convpass_swin_hypernet_linconv
from mmdet.models.adapters.convpass import forward_sam_block_t1,forward_sam_block_t5,forward_sam_block_t6
import torch.nn as nn

@MODELS.register_module()
class ATSSHyperConvpasslin(SingleStageDetector):
    ...
    def __init__(self,
        ...
        self.lin_type_embedding = nn.Embedding(2, self.layer_embedding_size).cuda()
    ...
    def apply_convpass(self,tuning_module):
        ...
        if self.method == 'attn+mlp':
            ...
            self.adapter_conv_hypetnet = HyperNetwork_conv(z_dim=self.layer_embedding_size,f_size=3,in_size=self.dim,out_size=self.dim,init_type=self.init_type).cuda()
            self.adapter_lin_hypernet = HyperNetwork_conv(z_dim=self.layer_embedding_size,f_size=1,in_size=self.backbone_embed_dim,out_size=self.dim,init_type=self.init_type).cuda()
            ...
            for child_module in tuning_module.children():
                if type(child_module) == mmengine.model.base_module.ModuleList:
                    for idx,layer in enumerate(child_module):
                            ...
                            layer.adapter_attn = Convpass_swin_hypernet_linconv(self.adapter_conv_hypetnet,self.adapter_lin_hypernet,attn_layer_embedding, self.lin_type_embedding).to(layer.norm1.weight.device)
                            layer.adapter_mlp =  Convpass_swin_hypernet_linconv(self.adapter_conv_hypetnet,self.adapter_lin_hypernet,mlp_layer_embedding, self.lin_type_embedding).to(layer.norm1.weight.device)
                            layer.s = self.scale
            ...
            for child_module in tuning_module.children():
                if type(child_module) == mmengine.model.base_module.ModuleList:
                    for idx,layer in enumerate(child_module):
                            ...
                            layer.adapter_attn = Convpass_swin_hypernet_linconv(self.adapter_conv_hypetnet,self.adapter_lin_hypernet,attn_layer_embedding, self.lin_type_embedding).to(layer.norm1.weight.device)
                            layer.adapter_mlp =  Convpass_swin_hypernet_linconv(self.adapter_conv_hypetnet,self.adapter_lin_hypernet,mlp_layer_embedding, self.lin_type_embedding).to(layer.norm1.weight.device)
                            layer.adapter_all = Convpass_swin_hypernet_linconv(self.adapter_conv_hypetnet,self.adapter_lin_hypernet,all_layer_embedding, self.lin_type_embedding).to(layer.norm1.weight.device)
                            layer.s = self.scale
                            bound_method = forward_sam_block_t6.__get__(layer, layer.__class__)
                            setattr(layer, 'forward', bound_method)
